...
export default function EMIOutcomeModal({ data, language, verified, onContinue, onVerify, onSkip, onLater, onClose }) {
  ...
  return <div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#102c23]/55 p-0 sm:items-center sm:p-5">
    <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-2xl bg-white shadow-2xl sm:rounded-2xl">
      <div className="flex items-start justify-between border-b border-slate-100 p-5 sm:p-7"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#0b6b45]">দুর্জয় · Outcome</p><h2 className="mt-1 text-2xl font-bold text-[#163b2d]">{bn ? 'আপনার ঋণের সম্ভাব্য ফলাফল' : 'Your loan outcome'}</h2><p className="mt-1 text-sm text-slate-500">{bn ? 'আপনার ব্যবসার তথ্য অনুযায়ী অনুমান' : 'Estimate based on your business information'}</p></div><button onClick={onClose} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100" aria-label="Close"><X /></button></div>
        <div className="flex items-start justify-between border-b border-slate-100 p-5 sm:p-7"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#0b6b45]">দুর্জয় · Outcome</p><h2 className="mt-1 text-2xl font-bold text-[#163b2d]">{bn ? 'আপনার ঋণের সম্ভাব্য ফলাফল' : 'Your loan outcome'}</h2><p className="mt-1 text-sm text-slate-500">{bn ? 'আপনার ব্যবসার তথ্য অনুযায়ী অনুমান' : 'Estimate based on your business information'}</p></div><button onClick={onClose} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100" aria-label="Close"><X /></button></div>
